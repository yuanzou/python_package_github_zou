...
if 0:
    ...
    @asyncio.coroutine
    def get_html(url, name, delay):
        print("%s get %s html start" % (name, url),time.time())
        yield from asyncio.sleep(delay)
        print("%s get %s html end" % (name, url),time.time())
    ...
    if __name__ == '__main__':
        ...
        # 创建两个协程
        tasks = [
            get_html("http://www.baidu.com", "A",0.1),
            get_html("http://www.souhu.com", "B",2),
        ]
        # 启动事件循环并将协程放进去执行
        # loop.run_until_complete(asyncio.wait(tasks))
        loop.run_until_complete(asyncio.gather(*tasks))
        # 刚刚看了一下文档，大概理解就是，async.wait会返回两个值:done和pending，done为已完成的协程Task，pending为超时未完成的协程Task，需通过future.result调用Task的result。而async.gather返回的是已完成Task的result。
